...
struct Font
{
    ...
    glm::vec2 drawString(float x, float y, const char *text, std::shared_ptr<lumina::Shader> font_shader_ptr)
    {
        glBindVertexArray(fontVao);
        glBindBuffer(GL_ARRAY_BUFFER, fontVertexBufferId);
        ...
        glBindTexture(GL_TEXTURE_2D, fontTexture);
        glUniform1i(glGetUniformLocation(font_shader_ptr->shader_id, "u_texture"), 0);
